...
export default function diff(virtualDOM, container, oldDOM) {
  ...
  const oldComponent =
    oldVirtualDOM && (oldVirtualDOM.component || oldVirtualDOM.fn);
  // 判断 oldDOM 是否存在
  if (!oldDOM) {
    ...
    // 如果替换的是组件 那么调用卸载当生命周期
    if (oldVirtualDOM.component) {
      oldVirtualDOM.component.componentWillUnmount();
    }
    ...
    if (oldChildNodes.length > virtualDOM.children.length) {
      if (hasNoKey) {
        ...
        // }
        for (let n = 0; n < virtualDOM.children.length; n++) {
          const key = virtualDOM.children[n].props.key;
          if (keyedElements[key]) {
            delete keyedElements[key];
          }
        }
        Object.keys(keyedElements).forEach((key) => {
          unmountNode(keyedElements[key]);
        });
      }
    }
  }
